...
pub mod pallet {
	...
	// define the struct of dot price
  #[derive(Deserialize, Encode, Decode, Default, RuntimeDebug)]
	...
	// define the meta struct of dot price
  ...
  #[derive(Deserialize, Encode, Decode, Default)]
	...
	// define the fmt function for debug code
  impl fmt::Debug for PriceMetaInfo {
		// `fmt` converts the vector of bytes inside the struct back to string for
		//   more friendly display.
		fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
			write!(
	...
	impl<T: Config> Pallet<T> {
        ...
        Ok(price_meta_info) => {
          let price_string = str::from_utf8(&price_meta_info.data.price_usd).map_err(|_| <Error<T>>::ParseError)?;
          let result: Vec<&str> = price_string.split('.').collect();

          let price_u64: u64 = result[0].parse::<u64>().map_err(|_| <Error<T>>::ParseError)?;

          let new_substring = result[1].get(0..6).unwrap();
		...
		fn fetch_from_remote_polkadot_price() -> Result<Vec<u8>, Error<T>> {
			const POLKADOT_PRICE_API: &str = "https://api.coincap.io/v2/assets/polkadot";
