use super::*;
use crate::mock::{new_test_ext, Event as TestEvent, KittyModule, Origin, System, Test};
use crate::Error;
...
macro_rules! assert_event {
	($x:expr) => {
		System::assert_has_event(TestEvent::KittyModule($x))
	};
}

fn new_kitty(account_id: u64) -> DispatchResult {
	KittyModule::create(Origin::signed(account_id))
}

#[test]
fn create_throw_max_count_overflow() {
...
fn create_failed_with_not_enough_balance() {
	new_test_ext().execute_with(|| {
		assert_noop!(new_kitty(3), Error::<Test>::NotSufficientFunds);
	});
}
...
fn transfer_fail_when_to_some_owner() {
	new_test_ext().execute_with(|| {
		let _ = new_kitty(1);
		assert_noop!(KittyModule::transfer(Origin::signed(1), 1, 1), Error::<Test>::NotSameOwner);
	});
}
...
fn transfer_fail_not_owner() {
	new_test_ext().execute_with(|| {
		let _ = new_kitty(1);
		assert_noop!(KittyModule::transfer(Origin::signed(2), 1, 1), Error::<Test>::NotOwnerOfKitty);
	});
}
...
fn breed_fail_with_same_kitty_id() {
	new_test_ext().execute_with(|| {
		assert_noop!(KittyModule::breed(Origin::signed(1), 1, 1), Error::<Test>::NotSameParentIndex);
	});
}
...
fn breed_fail_with_invalid_owner() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(KittyModule::breed(Origin::signed(1), 1, 2), Error::<Test>::NotOwnerOfKitty);
	});
}
...
fn sell_fail_with_not_owner() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			KittyModule::sell(Origin::signed(1), 1, Some(100)),
...
fn sell_success() {
	new_test_ext().execute_with(|| {
		let _ = new_kitty(1);
		assert_ok!(KittyModule::sell(Origin::signed(1), 1, Some(100)));
...
fn cancel_sell_with_none_price() {
	new_test_ext().execute_with(|| {
		let _ = new_kitty(1);
		let _ = KittyModule::sell(Origin::signed(1), 1, Some(100));

		assert_ok!(KittyModule::sell(Origin::signed(1), 1, None));
...
fn buy_failed_when_already_owned() {
	new_test_ext().execute_with(|| {
		let _ = new_kitty(1);
		assert_noop!(KittyModule::buy(Origin::signed(1), 1), Error::<Test>::KittyAlreadyOwned);
	});
}
...
fn buy_fail_when_not_for_sale() {
	new_test_ext().execute_with(|| {
		let _ = new_kitty(1);
		assert_noop!(KittyModule::buy(Origin::signed(2), 1), Error::<Test>::NotForSale);
	});
}
...
fn buy_fail_with_not_enough_balance() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(KittyModule::buy(Origin::signed(3), 1), Error::<Test>::NotSufficientFunds);
	});
}
...
fn buy_success() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(KittyModule::buy(Origin::signed(2), 1));
